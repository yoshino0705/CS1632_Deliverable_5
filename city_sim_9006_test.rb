...
# Refactor code from deliverable 2
# Test class for city_sim
class CitySimTest < Minitest::Test
  ...
  #
  #   Unknown combinations of 'for' and 'to' or
  #   unknown input values for either 'from' and 'to'
  #   -> returns 'Fifth Ave'
  ...
  def test_via_random_string
    assert_equal 'Fifth Ave', @driver.via('avc', 'dsads')
  end

  # UNIT TESTS FOR METHOD next_location
  ...
  def test_update_values_book
    cs = CitySim.new 0
    def cs.next_location
      'Hillman'
    end
    cs.update_values(cs.next_location)
